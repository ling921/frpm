...
打开 `http://服务器地址:8080`。首次登录账号为 `admin`，初始密码为 `Frpm@123`；请立即修改密码。
...
| 环境变量 | 默认值 | 用途 |
| --- | --- | --- |
| `ASPNETCORE_URLS` | `http://+:8080` | 容器内监听地址与端口。 |
...
示例：将宿主机 9000 端口映射到容器的默认 8080 端口：

```bash
docker run -d -p 9000:8080 -v frpm-data:/data tomcn/frpm:latest
```
